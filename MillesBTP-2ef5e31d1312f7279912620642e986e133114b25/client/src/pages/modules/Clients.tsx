...
interface ClientWithWorksitesCount extends Client {
  worksites_count: number;
}

export default function Clients() {
  const { session } = useSession();
  const [clients, setClients] = useState<ClientWithWorksitesCount[]>([]);
  ...
  async function loadClients() {
    ...
    try {
      ...
      // Charger le nombre de chantiers pour chaque client
                ...
                >
                  ...
                  <div className="text-right">
                    <Link href={`/dashboard/clients/${client.id}`}>
                      <div className="font-bold text-white hover:text-[#00ff88] cursor-pointer transition-colors">
                        {client.worksites_count}
                      </div>
                      <div className="text-xs text-gray-400">Chantier{client.worksites_count > 1 ? "s" : ""}</div>
                    </Link>
                  </div>
                  <Link href={`/dashboard/clients/${client.id}`}>
                    <Button variant="outline" className="border-[#3a3f47] text-gray-300 hover:bg-[#2f343a]">
                      Voir les chantiers
                    </Button>
                  </Link>
                </div>
